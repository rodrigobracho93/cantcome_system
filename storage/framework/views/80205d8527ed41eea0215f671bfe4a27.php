...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> Control de Stock <?php $__env->endSlot(); ?>

    <div class="flex justify-end mb-6">
        <a href="<?php echo e(route('stock-movements.create')); ?>" class="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors text-sm font-semibold">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-6">
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-lg font-bold text-gray-900 dark:text-white"><?php echo e(number_format($stats['total'])); ?></p>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-lg font-bold text-emerald-600 dark:text-emerald-400"><?php echo e(number_format($stats['units'])); ?></p>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4 flex items-center gap-3">
            ...
            <div>
                ...
                <p class="text-lg font-bold text-amber-600 dark:text-amber-400"><?php echo e($stats['products']); ?></p>
            </div>
        </div>
    </div>
    ...
    <div x-data="{
        query: '',
        results: [],
        selectedId: <?php echo e(request('product_id') ?: 'null'); ?>,
        selectedName: '<?php echo e(optional(App\Models\Product::find(request('product_id')))->name ?? ''); ?>',
        open: false,
        loading: false,
        async search() {
            if (this.query.length < 2) { this.results = []; this.open = false; return; }
            this.loading = true;
            const res = await fetch('<?php echo e(route("products.search")); ?>?q=' + encodeURIComponent(this.query));
        ...
        <form method="GET" class="flex flex-col sm:flex-row items-end gap-3">
            ...
            <div class="flex gap-2 w-full sm:w-auto">
                ...
                <?php if(request()->hasAny(['month', 'year', 'product_id'])): ?>
                <a href="<?php echo e(route('stock-movements.index')); ?>" class="px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors text-sm font-medium">Limpiar</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        ...
                        <td class="px-4 py-2.5 text-sm text-emerald-600 dark:text-emerald-400 text-right font-semibold">+<?php echo e(number_format($m->quantity)); ?></td>
                        ...
                        <td class="px-4 py-2.5 text-sm text-center">
                            <div class="flex items-center justify-center gap-1">
                                <a href="<?php echo e(route('stock-movements.edit', $m)); ?>" class="p-1.5 text-indigo-500 hover:text-indigo-700 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors" title="Editar">
    ...
    <div class="block md:hidden space-y-3">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
            <div class="flex items-center justify-between mb-3">
                <span class="text-sm font-semibold text-gray-900 dark:text-white"><?php echo e($m->product->name); ?></span>
                <span class="px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300">+<?php echo e(number_format($m->quantity)); ?></span>
            </div>
            <div class="grid grid-cols-2 gap-2 text-sm mb-3">
                <div>
                    <p class="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wider">Motivo</p>
                    <p class="font-medium text-gray-700 dark:text-gray-300"><?php echo e($m->motivo ?? '-'); ?></p>
                </div>
                <div>
                    <p class="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wider">Registrado por</p>
                    <p class="text-gray-700 dark:text-gray-300"><?php echo e($m->user?->name ?? '-'); ?></p>
                </div>
            </div>
            <div class="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-700">
                <p class="text-xs text-gray-400 dark:text-gray-500"><?php echo e($m->created_at->format('d/m/Y H:i')); ?></p>
                <div class="flex items-center gap-2">
                    <a href="<?php echo e(route('stock-movements.edit', $m)); ?>" class="inline-flex items-center gap-1 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg text-xs font-semibold hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors">
                        Editar
                    </a>
                    <form action="<?php echo e(route('stock-movements.destroy', $m)); ?>" method="POST" onsubmit="return confirm('¿Eliminar este movimiento? Se restarán <?php echo e($m->quantity); ?> del stock.')">
                        <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                        <button type="submit" class="p-1.5 text-red-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors" title="Eliminar">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                        </button>
