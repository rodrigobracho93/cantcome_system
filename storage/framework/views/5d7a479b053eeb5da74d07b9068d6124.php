...
<?php $component->withAttributes([]); ?>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4 flex items-center gap-3 <?php echo e(Auth::user()->isAdmin() ? 'cursor-pointer hover:shadow-md transition-shadow' : ''); ?>">
            ...
            <div>
                ...
                <p class="text-lg font-bold text-orange-600 dark:text-orange-400"><?php echo e($stats['pendingPrices']); ?></p>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center justify-end gap-2 mb-4">
        <a href="<?php echo e(route('products.create')); ?>" class="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm font-medium shadow-sm shadow-indigo-200 dark:shadow-none">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 mb-6">
        <form method="GET" action="<?php echo e(route('products.index')); ?>" class="flex flex-col sm:flex-row gap-3">
            ...
            <select name="price_status" class="w-full sm:w-36 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg text-sm focus:border-indigo-500 focus:ring-indigo-500">
                ...
                <option value="pending" <?php echo e(request('price_status') == 'pending' ? 'selected' : ''); ?>>Pendientes</option>
                <option value="approved" <?php echo e(request('price_status') == 'approved' ? 'selected' : ''); ?>>Aprobados</option>
            </select>
            <?php endif; ?>
            ...
            <div class="flex gap-2">
                ...
                <?php if(request('search') || request('category_id') || request('price_status') || request('stock')): ?>
    ...
    <?php else: ?>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors <?php echo e($product->isPricePending() ? 'border-l-4 border-l-orange-400' : ''); ?>">
                            <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                                <div class="flex items-center gap-2">
                                    <?php echo e($product->name); ?>

                                    <?php if($product->description): ?>
                                    <span class="text-xs text-gray-400 dark:text-gray-500 font-normal">- <?php echo e(Str::limit($product->description, 40)); ?></span>
                                    <?php endif; ?>
                                </div>
                            </td>
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400"><?php echo e($product->category?->name ?? 'Sin categoría'); ?></td>
                            <td class="px-4 py-3 whitespace-nowrap text-xs text-gray-400 dark:text-gray-500 font-mono"><?php echo e($product->barcode ?? '-'); ?></td>
                            <td class="px-4 py-3 whitespace-nowrap text-right text-sm font-bold <?php echo e($product->isPriceApproved() ? 'text-indigo-600 dark:text-indigo-400' : 'text-orange-600 dark:text-orange-400'); ?>">₲ <?php echo e(number_format($product->price, 0, ',', '.')); ?></td>
                            <td class="px-4 py-3 whitespace-nowrap text-center">
                                ...
                                <div class="w-16 h-1.5 mx-auto mt-1 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden">
                                    <div class="h-full rounded-full transition-all duration-300 <?php echo e($product->stock > 10 ? 'bg-emerald-500' : ($product->stock > 0 ? 'bg-amber-500' : 'bg-red-500')); ?>" style="width: <?php echo e(min(($product->stock / 50) * 100, 100)); ?>%"></div>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-center">
                                <?php if($product->isPricePending()): ?>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-right">
                                <div class="flex items-center justify-end gap-1">
                                    <?php if(Auth::user()->isAdmin() && $product->isPricePending()): ?>
                                    <form action="<?php echo e(route('products.approve-price', $product)); ?>" method="POST" class="inline">
                                        <?php echo csrf_field(); ?>
                                    ...
                                    <?php endif; ?>
                                    <?php if(Auth::user()->isAdmin()): ?>
                                    <a href="<?php echo e(route('products.edit', $product)); ?>" class="p-1.5 text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded-lg transition-colors" title="Editar">
        ...
        <div class="block md:hidden space-y-3">
            <?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden <?php echo e($product->isPricePending() ? 'border-l-4 border-l-orange-400 dark:border-l-orange-500' : ''); ?>">
                <div class="px-4 pt-4 pb-3">
                    <div class="flex items-start justify-between mb-2">
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-0.5">
                                <h3 class="text-sm font-semibold text-gray-900 dark:text-white truncate"><?php echo e($product->name); ?></h3>
                                <?php if($product->isPricePending()): ?>
                            ...
                            <p class="text-xs text-gray-400 dark:text-gray-500 truncate"><?php echo e($product->category?->name ?? 'Sin categoría'); ?></p>
                        </div>
                    ...
                    <p class="text-[10px] text-gray-300 dark:text-gray-600 font-mono mb-2"><?php echo e($product->barcode); ?></p>
                    <?php endif; ?>
                    ...
                    <?php if($product->creator && $product->isPricePending()): ?>
                    <p class="text-[10px] text-gray-400 dark:text-gray-500 mb-2">Creado por <?php echo e($product->creator->name); ?></p>
                    <?php endif; ?>

                    <div class="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-700">
                        <div>
                            <p class="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wider">Precio</p>
                            <p class="text-base font-bold <?php echo e($product->isPriceApproved() ? 'text-indigo-600 dark:text-indigo-400' : 'text-orange-600 dark:text-orange-400'); ?>">₲ <?php echo e(number_format($product->price, 0, ',', '.')); ?></p>
                        </div>
...
<?php endif; ?>
                ...
                <div class="flex items-center justify-between">
                    <a href="<?php echo e(route('products.import.template')); ?>" class="inline-flex items-center gap-1.5 text-xs text-indigo-600 dark:text-indigo-400 hover:underline font-medium">
