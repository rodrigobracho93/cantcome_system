...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> Historial de Ventas <?php $__env->endSlot(); ?>

    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
        <p class="text-sm text-gray-500 dark:text-gray-400">Total: <?php echo e($sales->total()); ?> ventas</p>
        <a href="<?php echo e(route('sales.create')); ?>" class="self-stretch sm:self-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors text-sm font-medium shadow-sm shadow-indigo-200 dark:shadow-none">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                            <div class="flex items-center gap-1.5">
                                ...
                                <?php if($sale->relationLoaded('cajaMovimientos') && $sale->cajaMovimientos->isNotEmpty()): ?>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300"><?php echo e($sale->customer?->full_name ?? 'Consumidor Final'); ?></td>
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-center">
                            <span class="px-2.5 py-1 rounded-full text-xs font-medium <?php echo e($sale->status === 'completado' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' : ($sale->status === 'anulado' ? 'bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300' : 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300')); ?>">
                        ...
                        <td class="px-4 py-3 whitespace-nowrap text-right text-sm">
                            <div class="flex items-center justify-end gap-2">
                                <a href="<?php echo e(route('sales.show', $sale)); ?>" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-medium">Ver</a>
                                <?php if(Auth::user()->isSuperAdmin() && $sale->status !== 'anulado'): ?>
                                <button @click="$dispatch('open-edit-sale', { id: <?php echo e($sale->id); ?>, payment: '<?php echo e($sale->payment_type); ?>', notes: '<?php echo e($sale->notes ?? ''); ?>' })"
                                ...
                                <form action="<?php echo e(route('sales.destroy', $sale)); ?>" method="POST" class="inline" onsubmit="return confirm('¿Anular esta venta? Se restaurará el stock.')">
                                ...
                                <form action="<?php echo e(route('sales.force-destroy', $sale)); ?>" method="POST" class="inline" onsubmit="return confirm('¿Eliminar permanentemente la venta #<?php echo e($sale->id); ?>? Esta acción no se puede deshacer.')">
                                    <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4">
            ...
            <div class="grid grid-cols-2 gap-2 text-sm mb-3">
                <div>
                    ...
                    <p class="font-medium text-gray-900 dark:text-white"><?php echo e($sale->created_at->format('d/m/Y H:i')); ?></p>
                </div>
                <div>
                    <p class="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wider">Vendedor</p>
                    <p class="text-gray-700 dark:text-gray-300"><?php echo e($sale->user->name); ?></p>
                </div>
                <div>
                    <p class="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wider">Cliente</p>
                    <p class="text-gray-700 dark:text-gray-300 truncate"><?php echo e($sale->customer?->full_name ?? 'Consumidor Final'); ?></p>
                </div>
                <div>
                    <p class="text-[10px] text-gray-400 dark:text-gray-500 uppercase tracking-wider">Pago</p>
                    <span class="px-2 py-0.5 rounded-full text-xs font-medium <?php echo e($sale->payment_type === 'contado' ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300' : 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300'); ?>">
                        <?php echo e(ucfirst($sale->payment_type)); ?>
            ...
            <div class="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-gray-700">
                <p class="text-lg font-bold text-gray-900 dark:text-white">₲ <?php echo e(number_format($sale->total, 0, ',', '.')); ?></p>
                <div class="flex items-center gap-2">
                    <a href="<?php echo e(route('sales.show', $sale)); ?>" class="inline-flex items-center gap-1 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg text-xs font-semibold hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition-colors">
                        Ver
                    </a>
                    <?php if(Auth::user()->isSuperAdmin() && $sale->status !== 'anulado'): ?>
                    <button @click="$dispatch('open-edit-sale', { id: <?php echo e($sale->id); ?>, payment: '<?php echo e($sale->payment_type); ?>', notes: '<?php echo e($sale->notes ?? ''); ?>' })"
                    ...
                    <?php endif; ?>
                    <?php if(Auth::user()->isAdmin() && $sale->status !== 'anulado'): ?>
                    <form action="<?php echo e(route('sales.destroy', $sale)); ?>" method="POST" onsubmit="return confirm('¿Anular esta venta? Se restaurará el stock.')">
                        <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.input-label','data' => ['value' => 'Tipo de Pago']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['value' => 'Tipo de Pago']); ?>
...
<?php endif; ?>
                ...
                <div class="mb-5">
                    <?php if (isset($component)) { $__componentOriginale3da9d84bb64e4bc2eeebaafabfb2581 = $component; } ?>
<?php if (isset($attributes)) { $__attributesOriginale3da9d84bb64e4bc2eeebaafabfb2581 = $attributes; } ?>
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.input-label','data' => ['value' => 'Notas']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
<?php $component->withName('input-label'); ?>
<?php if ($component->shouldRender()): ?>
<?php $__env->startComponent($component->resolveView(), $component->data()); ?>
<?php if (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag): ?>
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['value' => 'Notas']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
<?php if (isset($__attributesOriginale3da9d84bb64e4bc2eeebaafabfb2581)): ?>
<?php $attributes = $__attributesOriginale3da9d84bb64e4bc2eeebaafabfb2581; ?>
...
<?php endif; ?><?php /**PATH C:\xampp\Proyectos\cantcome_system\resources\views/sales/index.blade.php ENDPATH**/ ?>