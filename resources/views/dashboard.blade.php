<x-app-layout>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div class="group relative bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 hover:shadow-md hover:border-indigo-200 dark:hover:border-indigo-700 transition-all duration-200">
            <div class="flex items-start justify-between mb-3">
                ...
                <span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-semibold {{ $data['salesTrend'] > 0 ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' : 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400' }}">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $data['salesTrend'] > 0 ? 'M5 10l7-7m0 0l7 7m-7-7v18' : 'M19 14l-7 7m0 0l-7-7m7 7V3' }}"/>
                    </svg>
                    {{ abs($data['salesTrend']) }}%
                </span>
                @endif
        ...
        <div class="group relative bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 hover:shadow-md hover:border-emerald-200 dark:hover:border-emerald-700 transition-all duration-200">
            <div class="flex items-start justify-between mb-3">
                ...
                <span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-semibold {{ $data['revenueTrend'] > 0 ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' : 'bg-red-50 dark:bg-red-900/30 text-red-600 dark:text-red-400' }}">
                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $data['revenueTrend'] > 0 ? 'M5 10l7-7m0 0l7 7m-7-7v18' : 'M19 14l-7 7m0 0l-7-7m7 7V3' }}"/>
                    </svg>
                    {{ abs($data['revenueTrend']) }}%
                </span>
                @endif
            </div>
        ...
        <div class="group relative bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-5 hover:shadow-md hover:border-amber-200 dark:hover:border-amber-700 transition-all duration-200">
            <div class="flex items-start justify-between mb-3">
                ...
                <span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-md text-[10px] font-semibold bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400">
                    ...
                    Pendiente{{ $data['pendingSales'] !== 1 ? 's' : '' }}
                </span>
                @endif
            </div>
                ...
                <div class="flex items-end justify-between">
                    ...
                    <div class="w-16">
                        <div class="bg-red-200 dark:bg-red-900/40 rounded-full h-1.5 overflow-hidden">
                            <div class="h-full rounded-full {{ $product->stock == 0 ? 'bg-red-500' : 'bg-amber-500' }}" style="width: {{ min(($product->stock / 5) * 100, 100) }}%"></div>
                        </div>
                    </div>
                </div>
            </div>
    ...
    {{-- Charts Section --}}
    @if(Auth::user()->isAdmin() && isset($data['weeklySales']))
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            @php $maxQty = $data['topProducts']->max('total_qty') ?: 1; @endphp
            <div class="space-y-3">
                @forelse($data['topProducts'] as $i => $product)
                <div class="relative">
                    ...
                    <div class="w-full bg-gray-100 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                        <div class="h-full rounded-full transition-all duration-500 {{ $i === 0 ? 'bg-gradient-to-r from-amber-400 to-amber-500' : ($i === 1 ? 'bg-gradient-to-r from-gray-400 to-gray-500' : ($i === 2 ? 'bg-gradient-to-r from-amber-700 to-amber-800' : 'bg-indigo-400')) }}" style="width: {{ ($product->total_qty / $maxQty) * 100 }}%"></div>
                    </div>
                </div>
                @empty
    ...
    {{-- Payment Distribution + Quick Actions --}}
    @if(Auth::user()->isAdmin() && isset($data['salesByPayment']) && $data['salesByPayment']->isNotEmpty())
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            <div class="space-y-3">
                ...
                @php $pct = $totalPayments > 0 ? round(($payment->count / $totalPayments) * 100) : 0; @endphp
                <div>
                    <div class="flex items-center justify-between mb-1.5">
                        <span class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ $paymentLabels[$payment->payment_type] ?? $payment->payment_type }}</span>
                        <span class="text-xs font-semibold {{ $paymentTextColors[$payment->payment_type] ?? 'text-gray-600' }}">{{ $payment->count }} ventas</span>
                    </div>
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <a href="{{ route('sales.create') }}" class="group relative flex items-center gap-4 p-4 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white rounded-xl hover:from-indigo-600 hover:to-indigo-700 transition-all duration-200 hover:scale-[1.02] hover:shadow-lg hover:shadow-indigo-200 dark:hover:shadow-none">
                ...
                <a href="{{ route('sales.index') }}" class="group relative flex items-center gap-4 p-4 bg-gradient-to-r from-gray-700 to-gray-800 text-white rounded-xl hover:from-gray-800 hover:to-gray-900 transition-all duration-200 hover:scale-[1.02] hover:shadow-lg">
                    <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-white/20 shrink-0 group-hover:scale-110 transition-transform">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>
                        </svg>
                ...
                <a href="{{ route('products.index') }}" class="group relative flex items-center gap-4 p-4 bg-gradient-to-r from-emerald-500 to-emerald-600 text-white rounded-xl hover:from-emerald-600 hover:to-emerald-700 transition-all duration-200 hover:scale-[1.02] hover:shadow-lg hover:shadow-emerald-200 dark:hover:shadow-none">
                    <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-white/20 shrink-0 group-hover:scale-110 transition-transform">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/>
                        </svg>
                ...
                <a href="{{ route('admin.users') }}" class="group relative flex items-center gap-4 p-4 bg-gradient-to-r from-purple-500 to-purple-600 text-white rounded-xl hover:from-purple-600 hover:to-purple-700 transition-all duration-200 hover:scale-[1.02] hover:shadow-lg">
                    <div class="flex items-center justify-center w-10 h-10 rounded-lg bg-white/20 shrink-0 group-hover:scale-110 transition-transform">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <script>
        ...
        new Chart(ctx, {
            type: 'bar',
            data: {
                labels: {!! json_encode($data['weeklySales']->pluck('date')->map(fn($d) => \Carbon\Carbon::parse($d)->isoFormat('dddd'))) !!},
                datasets: [{
                    label: 'Ventas (₲)',
                    data: {!! json_encode($data['weeklySales']->pluck('total')) !!},
