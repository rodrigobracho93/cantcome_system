<x-app-layout>
    ...
    {{-- Filtros --}}
    <form method="GET" action="{{ route('sales.index') }}" class="mb-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4"
        x-data="{
            query: '{{ $selectedCustomer?->full_name ?? '' }}',
            ...
            async search() {
                ...
                this.loading = true;
                const res = await fetch('{{ route("customers.search") }}?q=' + encodeURIComponent(this.query));
        ...
        <div class="flex flex-col sm:flex-row items-end gap-3">
            ...
            <div class="flex gap-2 w-full sm:w-auto">
                ...
                @if(request()->hasAny(['month', 'year', 'customer_id']))
                <a href="{{ route('sales.index') }}" class="px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors text-sm font-medium">Limpiar</a>
                @endif
            </div>
        </div>
    </form>
